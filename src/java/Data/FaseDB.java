...
public class FaseDB {
    ...
    public static void insert(Fase fase) {
        ...
        int[] fechaInicio = getFechaInt(fase.getFechaInicio());
        diaInicio = fechaInicio[0];
        mesInicio = fechaInicio[1];
        anoInicio = fechaInicio[2];
        int[] fechaFin = getFechaInt(fase.getFechaFin());
        diaFin = fechaFin[0];
        mesFin = fechaFin[1];
        anoFin = fechaFin[2];
        String query = "INSERT INTO Fases (nombre, diaInicio, mesInicio, anoInicio, diaFin, mesFin, anoFin, estado, idProyecto) VALUES ('"
                + fase.getNombre() + "',"
                + diaInicio + ","
                + mesInicio + ","
                + anoInicio + ","
    ...
    public static ArrayList<Fase> selectFases(int idProyecto) {
        ...
        PreparedStatement ps = null;
        ResultSet rs = null;
        String query = "SELECT * FROM Fases WHERE idProyecto=? ORDER BY anoInicio, mesInicio, diaInicio ASC";
        ...
        try {
            ...
            while(rs.next()){
                ...
                Fase f = new Fase(rs.getInt(1), rs.getString(2), fechaInicio, fechaFin, rs.getString(9).charAt(0), idProyecto);
                fases.add(f);
            }
            rs.close();
            ps.close();
